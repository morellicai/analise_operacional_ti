...
from etl.metrics import soma_finalizados, cards_em_andamento, cards_finalizados_count_by_priority
...
def kpis(df):
    ...
    with col1:
        st.metric(
            ...
            value=soma_finalizados(df)
        )
    with col2:
        st.metric(
            label='Volume de Cards em Andamento', 
            value=cards_em_andamento(df)
        )
    with col3:
        st.metric(
    ...
    with col4:
        st.metric(
            label='Volume de Cards Finalizados por prioridade Alta e Crítico / Urgente', 
